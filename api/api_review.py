...
class Reviews(Resource):
    ...
    def post(self):
        ...
        is_valid, message = data_manager.validate_review_data(new_review_data)
        if not is_valid:
            ns.abort(400, message)

        new_review_data['id'] = str(uuid.uuid4())
        new_review_data['created_at'] = datetime.now().isoformat()
        new_review_data['updated_at'] = datetime.now().isoformat()
        review = Review(**new_review_data)
        review_id = data_manager.save_review(review.to_dict())
        return {
