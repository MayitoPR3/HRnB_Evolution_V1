...
from model.places import Place  # Ensure this import is correct
import uuid
...
class Places(Resource):
    ...
    def post(self):
        ...
        is_valid, message = data_manager.validate_place_data(new_place_data)
        if not is_valid:
            ns.abort(400, message)

        new_place_data['id'] = str(uuid.uuid4())
        new_place_data['created_at'] = datetime.now().isoformat()
        new_place_data['updated_at'] = datetime.now().isoformat()
        place = Place(**new_place_data)
        data_manager.save_place(place.to_dict())
        return {
            'message': 'Place created successfully',
            'place_id': new_place_data['id']
        }, 201
